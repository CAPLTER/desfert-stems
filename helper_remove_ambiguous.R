...
#'   identity of the plants (e.g., which one is actually L2 and which is
#'   whatever plant was mislabeled as L2). However, more typically, this is an
...
#'   removes records associated with a twice-labeled plant at a given plot,
...
#'
#' @param plot (integer) The plot (as plot id) where the plant are mislabeled.
#' @param duplicated_plant (character) The identity of the plant that is
#'   duplicated. For example, if there are two plants labeled L2 for a given
#'   plot, that would be indicated with this parameter as "L2".
#' @param missing_plant (character) The identity of the plant that has been
#'   omitted. For example, if there are two plants labeled L2 for a given plot,
#'   this parameter would correspond to the identify of the plant that was
#'   mislabeled (i.e., that is missing from the record for the plot). Extending
#'   the example, if there were two plants labeled L2 and none of the plants
#'   were labeled L3 then L3 would be the missing plant and would be passed as
#'   an argument to this parameter.
#' @param survey_date (character) The date (ISO format as character) that the
#'   plot was surveyed.
#'
